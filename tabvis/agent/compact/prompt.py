"""Compact (conversation-summary) prompt helpers.

``PartialCompactDirection`` is typed as a free string; the runtime values used by
``get_partial_compact_prompt`` are ``'from'`` (default) and ``'up_to'``, and the
function branches on ``'up_to'``.
"""
...
# Direction is effectively a free string at runtime.
...
# Pre-compiled regexes. [\s\S] matches any character including newlines, giving
# a dotall-style match across the tag body.
...
def format_compact_summary(summary: str) -> str:
    ...
    :param summary: The raw summary string potentially containing <analysis> and
        <summary> XML tags.
    :returns: The formatted summary with analysis stripped and summary tags
        replaced by headers.
    """
    ...
    formatted_summary = _ANALYSIS_RE.sub("", formatted_summary, count=1)

    # Extract and format summary section.
    summary_match = _SUMMARY_RE.search(formatted_summary)
    if summary_match:
        content = summary_match.group(1) or ""
        formatted_summary = _SUMMARY_RE.sub(
            lambda _m: f"Summary:\n{content.strip()}",
            formatted_summary,
            count=1,
        )
    ...
    formatted_summary = _BLANK_LINES_RE.sub("\n\n", formatted_summary)
...
def get_compact_user_summary_message(
    summary: str,
    suppress_follow_up_questions: bool | None = None,
    transcript_path: str | None = None,
    recent_messages_preserved: bool | None = None,
) -> str:
